...
运行方式：
    python sym_search_2d_gaas.py config.json
...
def build_local_env_config(args: argparse.Namespace) -> LocalEnvConfig:
    ...
    raw_cfg = getattr(args, "local_env_constraints", {}) or {}
    if not isinstance(raw_cfg, dict):
        raise TypeError("local_env_constraints 需为对象，例如 {\"elements\": {\"Ga\": {...}}}")
    raw_cfg = dict(raw_cfg)

    # 兼容旧格式：{"defaults": {...}, "rules": [...]}
    legacy_defaults = raw_cfg.get("defaults")
    legacy_rules = raw_cfg.get("rules")
    if legacy_defaults or legacy_rules:
        if "global_cn_range" not in raw_cfg and legacy_defaults:
            raw_cfg["global_cn_range"] = legacy_defaults.get("cn_range")
        if "threshold" not in raw_cfg and legacy_defaults:
            raw_cfg["threshold"] = legacy_defaults.get("threshold", 0.65)
        if "default_motifs" not in raw_cfg and legacy_defaults:
            raw_cfg["default_motifs"] = (
                legacy_defaults.get("preferred_motifs")
                or legacy_defaults.get("motifs")
                or []
            )
        # rules → elements
        elems = raw_cfg.get("elements") or {}
        if legacy_rules and not elems:
            elems = {}
            for rule in legacy_rules:
                if not isinstance(rule, dict):
                    continue
                species = rule.get("species") or []
                cn_range = rule.get("cn_range")
                motifs = (
                    rule.get("preferred_motifs")
                    or rule.get("motifs")
                    or []
                )
                for sp in species:
                    ...
                    if not sp_key:
                        continue
                    e = elems.setdefault(sp_key, {})
                    if cn_range is not None:
                        e["cn_range"] = cn_range
                    if motifs:
                        e.setdefault("preferred_motifs", motifs)
        if elems and "elements" not in raw_cfg:
            raw_cfg["elements"] = elems
...
def load_settings(config_path: str) -> argparse.Namespace:
    ...
    # local_env_constraints 原样存 dict，真正解析交给 build_local_env_config
...
class CandidateEvaluator:
    ...
    def evaluate(self,
        ...
        if self.local_env_checker is not None and self.local_env_config is not None and self.local_env_config.enabled:
        ...
        # 4. 转原胞 + 再做一次 slab 投影（保证输出是标准原胞 slab）
        prim = to_primitive(
            struct,
            symprec=sym_used or self.symprecs[0],
            angle_tolerance=self.args.angle_tol,
        )
